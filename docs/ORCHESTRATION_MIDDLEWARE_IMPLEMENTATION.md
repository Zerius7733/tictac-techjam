...
Migration `009_waiting_agent_runs.sql` rebuilds `agent_runs` with the expanded
...
`db/migrations/009_waiting_agent_runs.sql`,
`db/migrations/010_archived_agents.sql`,
...
upgrade is migration `011_data_asset_permissions.sql`; the remaining gaps are
...
| 2026-08-30 | 3 (complete) / 8 (partial) | Archived Agents now retain their database row, runs, messages, and workspace archive instead of deleting history. Added migration `010_archived_agents.sql`. Added repository and dispatcher cancellation, conservative restart reconciliation, per-run/job timeouts, correlated runner request IDs, diagnostic redaction, and structured lifecycle logging. | Focused operational suite: 31 tests passed; server typecheck passed. Public orchestration cancel routes, durable authenticated waiting-run resume, and real API wiring remain pending. |
