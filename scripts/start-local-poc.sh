...
load_poc_env_file() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$key" in
      ARK_API_KEY|ARK_MODEL|ARK_BASE_URL|CODEX_SANDBOX_MODE|CODEX_TIMEOUT_MS|CODEX_MAX_OUTPUT_BYTES|ORCHESTRATION_RUN_TIMEOUT_MS|ORCHESTRATION_JOB_TIMEOUT_MS|CONTAINER_ENGINE|CONTAINER_RUNTIME_IMAGE|CONTAINER_RUNTIME_BASE_IMAGE|CONTAINER_APT_MIRROR|CONTAINER_APT_SECURITY_MIRROR|CONTAINER_RUNTIME_APT_PACKAGES|CONTAINER_CPU_LIMIT|CONTAINER_MEMORY_LIMIT|CONTAINER_PIDS_LIMIT|CONTAINER_USER|RUNTIME_INSTANCE_ID|LOCAL_POC_DATA_ROOT|SEED_DEVELOPMENT_DATA)
        ;;
      *)
        continue
        ;;
...
# The local POC keeps production-style Runtime behavior, but should still
# provision the deterministic development accounts, protected resource catalog,
# and Order Dashboard project on a fresh persistent state directory. Set
# SEED_DEVELOPMENT_DATA=false when an empty local database is intentional.
export SEED_DEVELOPMENT_DATA="${SEED_DEVELOPMENT_DATA:-true}"
