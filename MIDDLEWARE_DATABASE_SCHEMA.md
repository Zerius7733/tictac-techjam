...
003_agent_principals.sql               # independent Agent identities
...
release, where policy and credential files were recorded as versions `004` and
`005`. It applies the waiting/archive upgrades and records their names in a
small compatibility ledger, so existing policy tables and audit history are
retained.
